...
* Date: 17/12/2021
* Description: Actions and functions implementation of api interface.
*/
#include <stdio.h>
#include <math.h>
#include <string.h>
...
/**************** PR2 EX1A ****************/
 int length;
    ...
    while (index <= length && !found){
        bookingListGet(bookings,index,&booking);
        found = (booking.id == coworkerId);
        index++;
    }
	/*if the reservation is found, the index is returned, if not, the value -1*/
    if (found)
        index= index-1;
    else
        index= NO_BOOKING;
...
float calculatePrice(tCoworkingCenter coworkingCenter, tBooking booking, tDate date)
{
    float price= 0.0; 
/**************** PR2 EX2A ****************/    
 float singleWorkPlace, doubleWorkPlace, tripleWorkPlace, quadWorkPlace;
    int i, floor, nDays;

    /* get the price per workPlace */
    singleWorkPlace= coworkingCenter.price * SINGLE_WORKPLACE_PRICE_FACTOR;
    doubleWorkPlace= coworkingCenter.price * DOUBLE_WORKPLACE_PRICE_FACTOR;
    tripleWorkPlace= coworkingCenter.price * TRIPLE_WORKPLACE_PRICE_FACTOR;
    quadWorkPlace=  coworkingCenter.price * QUAD_WORKPLACE_PRICE_FACTOR;
    ...
    for (i= 0; i<booking.nAssignedSpaces; i++)
    {
        ...
        if (floor == 1)
            price += singleWorkPlace;
        else if (floor == 2)
            price += doubleWorkPlace;
        else if (floor == 3)
            price += tripleWorkPlace;
        else
            price += quadWorkPlace;
    }
 
 /* add price per person depending on the number of spaces assigned and the equipment required */
    ...
    /* product by number of days for this booking */
	nDays = 0;
...
tCoworkerId coworkerId;
    ...
    while (!isCoworkerQueueEmpty(*queue)){
        ...
        /*if the coworkerTableFind function does not return -1, 
         * the result of the findCoworkerBooking function is saved in the index variable*/
        if (coworkerTableFind(coworkers,coworkerId) != NO_COWORKER){
            index = findCoworkerBooking(*currentBookings,coworkerId);
            /*if index different from -1 we pass the reservation by pointer to booking*/
            if (index != NO_BOOKING){
                bookingListGet(*currentBookings,index, &booking);
                /*if the reservation date is the same as the current one*/
                if (dateCmp(booking.checkOutDate,date) == 0){
                    /*the reservation is removed from the current reservations list.*/
                    bookingListDelete(currentBookings,index);
                    price = calculatePrice(*coworkingCenter,booking,date);
                    /*the free spaces of the matrix are updated*/
                    freeSpacesOfBooking(coworkingCenter,booking);
                    booking.price = price;
                    /*the reservation is added to the completed list*/
...
tCoworkerId coworkerId;
    ...
    while (!isCoworkerQueueEmpty(*queue)){
        /*the identifier of the first queue coworker is sent through a pointer to the variable coworkerId*/
        coworkerQueueHead(*queue,&coworkerId);
        /*if the coworker finds the reservation then*/
        if (coworkerTableFind(coworkers,coworkerId) != NO_COWORKER){
            index = findCoworkerBooking(*pendingBookings, coworkerId);
        
            if (index != NO_BOOKING){
                bookingListGet(*pendingBookings,index,&booking);
                /*if the date is the same as the current date*/ 
                if (dateCmp(booking.checkInDate,date) == 0){
                    /* the reservation is deleted from the pending list*/
                    bookingListDelete(pendingBookings,index);
                    assignSpacesForBooking(coworkingCenter,&booking);
                    /*added to the list of reservations in progress*/
