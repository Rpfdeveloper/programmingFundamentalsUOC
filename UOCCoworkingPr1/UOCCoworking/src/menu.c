...
* Date: 29-11-2021
* Description: Actions and functions implementaction of the menu interface
*/
#include <stdio.h>
#include <string.h>
#include "menu.h"
...
/*************** PR1 EX2  *****************/
	/*Data input*/
	/*ID request*/
	printf("Coworker id  (integer between 1 and %d):\n>>",MAX_COWORKERS);
	scanf("%d",&number);
	/*Filters for entered data*/
	if (number > MAX_COWORKERS || number < ONE){
		*retVal = ERROR;
	} else {
		coworker->id = (tCoworkerId)number;
	}
	/*If the data is not as expected, the constant ERROR 
	 * is returned by output parameter,otherwise the content 
	 * of the buffer or number variable is copied per output parameter, 
	 * depending on the type of data.*/
	printf("Coworker name (max %d char, no spaces):\n>>",MAX_NAME - ONE);
	scanf("%s",buffer);	
	length = strlen(coworker->name);
	if (length < MAX_NAME){
		strncpy(coworker->name,buffer,MAX_NAME - ONE);
	} else {
		*retVal = ERROR;	
	}
	
	printf("Coworker surname(max %d char,no spaces):\n>>",MAX_SURNAME - ONE);
	scanf("%s",buffer);	
	length = strlen(coworker->surname);
	if (length < MAX_SURNAME){
		strncpy(coworker->surname,buffer,MAX_SURNAME - ONE);
	} else {
		*retVal = ERROR;
	}
	
	printf("Coworker doc number (max %d char, no spaces):\n>>",MAX_DOC_NUMBER-ONE);
	scanf("%s",buffer);
	length = strlen(coworker->docNumber);
	if (length < MAX_DOC_NUMBER){
		strncpy(coworker->docNumber,buffer,MAX_DOC_NUMBER - ONE);
	} else {
		*retVal = ERROR;
	}
	/*Data request for tDate type*/
	printf("Coworker birthdate --year:\n>>");
	scanf("%d",&coworker->birthDate.year);

	printf("Coworker birthdate --month:\n>>");
	scanf("%d",&coworker->birthDate.month);
	
	printf("Coworker birthdate --day:\n>>");
	scanf("%d",&coworker->birthDate.day);
	
	printf("Coworker birth city (max %d char,no spaces):\n>>",MAX_CITY);
	scanf("%s",buffer);
	length = strlen(coworker->birthCity);
	if (length < MAX_CITY){
		strncpy(coworker->birthCity,buffer,MAX_CITY - ONE);
	} else {
		*retVal = ERROR;
	}
	
	printf("Coworker civil state type (1 digit being 0=SINGLE, 1=MARRIED, 2=SEPARATED, 3=DIVORCED, 4=WIDOWED):\n>>");
	scanf("%d",&number);
	if (number >= ZERO && number <= FOUR){
		coworker->status = (tCivilState)number;
	} else {
		*retVal = ERROR;
	}
	
	printf("Coworker professional profile type (1 digit being 0=FREELANCER, 1=TELEWORKER, 2=ENTREPRENEUR, 3=OCCASIONAL, 4=EMPLOYEE, 5=PARTNER, 6=OTHER):\n>>");
	scanf("%d",&number);
	if (number >= ZERO && number <= SIX){
	...
	/*Initialite values*/
	coworker->bookingsCounter = ZERO;
	coworker->points = ZERO;
	coworker->satisfaction = ZERO;
...
void allCoworkersRecommendation(tAppData appData,char *city, float price, float distance, int points) { 

/*************** PR1 EX5B *****************/
	int i,j;
	tCoworkerTable recomendatedCoworkers;
	tCoworkingCenterTable recomendatedCenters;

	coworkerTableSelectCoworkers(appData.coworkers,city,&recomendatedCoworkers);
	
	for (i = ZERO;i < appData.coworkers.nCoworkers;i++){
		coworkerRecommendation(recomendatedCoworkers.table[i],appData.coworkingCenters,city,price,distance,points,&recomendatedCenters);
	
		if (recomendatedCoworkers.nCoworkers > ZERO){
			printf("COWORKER\n");
			printCoworker(recomendatedCoworkers.table[i],i);
	}
		for (j = ZERO;i < recomendatedCenters.nCoworkingCenters;j++){
			printf("\t");
			printCoworkingCenter(recomendatedCenters.table[i],j);
		}
	printf("\n");
} 
